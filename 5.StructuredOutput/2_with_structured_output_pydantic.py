...
class AnnotatedReview(BaseModel):
    # also validated the response
    key_themes: list[str] = Field(description='Write down the key themes discussed int the review in a list')
    summary: str = Field(description='A brief summary of the review')
    sentiment: Literal['Pos', 'Neg'] = Field(description='Return sentiment of the review either negative, positive')
    pros : Optional[list[str]] = Field(default=None, description='Write down pros inside a list')
    cons : Optional[list[str]] = Field(default=None, description='Write down cons inside a list')
    name : Optional[str] = Field(default=None, description='Write the name of riviewr')
